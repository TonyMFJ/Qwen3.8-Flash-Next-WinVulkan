...
struct llama_ple_disk::impl {
    ...
    impl(const std::string & fname, size_t offs, ggml_type type, int64_t ne0, int64_t nrows, const params & p)
        ...
        direct = p.direct_io;
#if defined(_WIN32)
        // MinGW has no O_DIRECT / O_CLOEXEC. Always use buffered reads (correct, no alignment headache).
        direct = false;
        fd = _open(fname.c_str(), _O_RDONLY | _O_BINARY);
        if (fd < 0) {
            throw std::runtime_error(format("llama_ple_disk: failed to open %s", fname.c_str()));
        }
#else
        if (direct) {
    ...
    uint8_t * alloc_bounce() const {
        void * ptr = nullptr;
#if defined(_WIN32)
        ptr = _aligned_malloc(bounce_size(), block);
        if (ptr == nullptr) {
    ...
    void pread_full(uint8_t * dst, size_t len, int64_t off, size_t need) const {
        ...
        while (got < len) {
#if defined(_WIN32)
            // MinGW: _lseeki64 + _read (each read seeks first, replicating pread semantics)
            errno = 0;
            const __int64 pos = (__int64) off + (__int64) got;
            if (_lseeki64(fd, pos, SEEK_SET) == (__int64) -1) {
                GGML_ABORT("llama_ple_disk: _lseeki64(%s, %lld) failed: %s",
                           fname.c_str(), (long long) pos, strerror(errno));
            }
            const int r = (int) _read(fd, dst + got, (unsigned) (len - got));
            if (r < 0) {
                if (errno == EINTR) {
                    continue;
                }
                GGML_ABORT("llama_ple_disk: _read(%s, %zu @ %lld) failed: %s",
    ...
    void read_row(int64_t row, uint8_t * dst, uint8_t * bounce) const {
        const int64_t off = (int64_t) offs + (int64_t) row * (int64_t) rs;
        if (!direct) {
